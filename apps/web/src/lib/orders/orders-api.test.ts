...
import { customerOrderCancelPath, customerOrderReturnPath, customerOrdersPath } from './orders-api';
...
test('encodes customer order action paths', () => {
  assert.equal(customerOrderCancelPath('NV/ABC 001'), '/v1/account/orders/NV%2FABC%20001/cancel');
  assert.equal(customerOrderReturnPath('NV/ABC 001'), '/v1/account/orders/NV%2FABC%20001/returns');
});
