...
- 2026-09-15 provider-free local test lane completed: development/test seed creates the synthetic admin fixture when `LOCAL_TEST_MODE=true`; the local staff factor helper, real HTTP staff login/me, and real HTTP customer OTP request/verify/me paths were verified without provider credentials. The full deterministic integration matrix passes `8/8` suites and focused local provider/config/auth checks pass `54/54`. This closes local functional testing for OTP/SMS behavior, payment, shipping and notification routing only; real accounts, sandbox credentials and live provider contracts remain open.

- 2026-09-15 OPS-002 real non-production recovery drill passed: PostgreSQL 16.15 clients plus a disposable Docker `postgres:16-alpine` target completed archive and separate-cluster restore from the read-only local `nova-local` source; `54` tables and `_prisma_migrations` were verified, with distinct cluster identities and exact cleanup. The verifier now handles multiple installed PostgreSQL majors by selecting the first application path from `PATH`. Production backup/restore, deployment and rollback evidence still require approved production/staging inputs and remain open.

- 2026-09-15 provider decision: production shipping is assigned to the Iran Post adapter boundary (`iran-post`). The official quote/ETA endpoint, payload contract and sandbox credential remain unverified, so the adapter is fail-closed and has no invented network request; development/test continue using local shipping.

- Latest parent validation (2026-09-14, second bounded sub-agent wave): customer OTP now rejects an inactive customer returned by the final upsert, staff password verification rejects an empty decoded digest, and account return responses are route-order scoped. The earlier CSRF/backup/cache work remains integrated. The focused second-wave run passes `40/40`; source/plain suites pass `450`/`715` with `1`/`2` opt-in MinIO skips (`451`/`717` total tests); deterministic integration passes `8` suites / `153` underlying tests; production build, local-release verification, live preflight `12/12`, configured signal probe `5/5` and elevated fixture-backed Chromium pass `68/68`, with typecheck/lint, targeted formatting and diff-check green. Repository-wide format-check remains red on `116` pre-existing files. Tapin's public material was reviewed read-only, but no authoritative quote/ETA endpoint and matching parcel contract was verified, so the provider remains fail-closed and unimplemented. No credentials, provider transaction, restore/deploy, commit, merge or push was used.

- Latest parent validation (2026-09-14, after the CSRF/backup/customer-cache wave): the CSRF boundary regression covers array-valued HTTP headers; archive-only backup verification rechecks source database/server identity around `pg_dump`; and customer OTP verification clears protected address/order caches before publishing the new customer while preserving a guest cart. The focused wave passes `29/29`, source/plain suites pass `448`/`712` with `1`/`2` opt-in MinIO skips, deterministic integration passes `8` suites / `152` underlying tests, and typecheck/lint, production build, local-release verification and the elevated fixture-backed Chromium matrix `68/68` pass. Public Tapin material was checked read-only: it documents Tipax order-registration and price-related fields, but no authoritative quote/ETA transport contract matching `TapinShippingTransport.quote()` was verified, so no speculative endpoint or payload was implemented. No credentials, provider transaction, restore/deploy, commit, merge or push was used; protected auth/provider/restore/deploy/AT/pixel/remote gates remain open.

Current parent revalidation (2026-09-14, after checkout/worker wave): source-scoped `bun run test` passes `445` with `1` opt-in MinIO skip and `0` failures across `78` files; plain `bun test` passes `707` with `2` opt-in MinIO skips and `0` failures across `121` files; checkout/worker focused tests pass `18/18`; deterministic integration passes `8` suites / `149` underlying tests; typecheck/lint, elevated production build, local-release verification, live preflight `12/12`, configured signal probe `5/5` and fixture-backed pinned-Chromium `68/68` pass. Targeted Prettier and `git diff --check` pass; repository-wide format-check remains red on `116` pre-existing files. No credentials, provider transaction, shared-data mutation, restore/deploy, commit, merge or push was used.

Current parent revalidation (2026-09-14, after checkout/worker wave): source-scoped `bun run test` passes `445` with `1` opt-in MinIO skip and `0` failures across `78` files; plain `bun test` passes `707` with `2` opt-in MinIO skips and `0` failures across `121` files; checkout/worker focused tests pass `18/18`; deterministic integration passes `8` suites / `149` underlying tests; typecheck/lint, elevated production build, local-release verification, live preflight `12/12`, configured signal probe `5/5` and fixture-backed pinned-Chromium `68/68` pass. Targeted Prettier and `git diff --check` pass; repository-wide format-check remains red on `116` pre-existing files. No credentials, provider transaction, shared-data mutation, restore/deploy, commit, merge or push was used.

Superseded parent revalidation (2026-09-14, before the second auth/account wave): source-scoped `bun run test` passed `443` with `1` opt-in MinIO skip and `0` failures across `78` files; plain `bun test` passed `704` with `2` opt-in MinIO skips and `0` failures across `121` files; the focused provider/auth/cart gate passed `30/30`; typecheck/lint, deterministic integration, elevated production build, local-release verification and fixture-backed pinned-Chromium `68/68` passed. The unprivileged build still hit the known Windows esbuild access boundary before source compilation; live preflight `12/12` and the five-signal probe passed. No credentials, provider transaction, restore/deploy, commit, merge or push was used; disposable validation databases were torn down and shared application data remained unchanged.

Superseded parent revalidation (2026-09-14, before the second auth/account wave): source-scoped `bun run test` passed `443` with `1` opt-in MinIO skip and `0` failures across `78` files; the plain `bun test` invocation passed `704` with `2` opt-in MinIO skips and `0` failures across `121` files; the provider/auth/cart focused gate passed `30/30`; workspace typecheck/lint and deterministic integration (8 suites / 148 underlying tests; 0 failures) passed. The elevated production build and local-release verifier passed for package/API/web-SSR/worker artifacts and reparse-point guards; the unprivileged build still hit the known Windows esbuild access boundary before source compilation. The fixture-backed elevated pinned-Chromium matrix remained `68/68` in `27` files. Separately, the live E2E preflight passed `12/12` and the configured read-only API/Redis/MinIO/worker signal probe passed all `5/5` checks against healthy local services. No credentials, provider transaction, restore/deploy, commit, merge or push was used; the reservation check temporarily mutated and restored the persistent local `nova` inventory baseline, while notification/media/concurrency checks used disposable databases/buckets. No shared application data remained mutated.

- Media continuation (2026-09-14): the opt-in `bun run test:media-live` harness passed the real local HTTP+Prisma+MinIO lifecycle on a unique loopback database/bucket with synthetic staff/TOTP and isolated Redis: CSRF/login, presign/complete, direct original/derivative PUT, `READY` persistence, public redirect, signed derivative read, authenticated delete, derivative-source `404`, zero-row cleanup and exact teardown all passed. Focused catalog/media tests pass `38/38`; real staff/MFA, external provider and production-storage evidence remain open.
- OPS-002 runbook follow-up (2026-09-14): the archive-only example now explicitly loads `NOVA_BACKUP_SOURCE_DATABASE_URL` through the approved current-process secret flow with a placeholder only. Parser, version/evidence/namespace/runbook and diff checks pass; PostgreSQL 16 archive/restore remains blocked by host 18.1 tools and the lack of an authorized separate target.
- Latest page-local parallel wave (2026-09-14): Pascal fixed the blank eligible return-status state in WEB-003; Hubble corrected selected inventory-detail loading/error precedence while preserving the disabled-query empty state in ADMIN-001; Fermat normalized support/finance role matching in ADMIN-003; and Carson made dirty content drafts fail closed before publish in ADMIN-004/WEB-005. Parent review added the disabled TanStack-query regression assertion, and the combined focused account/admin/content gate passes `61/61` with web typecheck and formatting. Live authenticated, provider, storage, browser-pixel and full-AT gates remain separate.
- Latest page-local parallel wave (2026-09-14): Pascal fixed the blank eligible return-status state in WEB-003; Hubble corrected selected inventory-detail loading/error precedence while preserving the disabled-query empty state in ADMIN-001; Fermat normalized support/finance role matching in ADMIN-003; and Carson made dirty content drafts fail closed before publish in ADMIN-004/WEB-005. Galileo then aligned staff-login client validation with the API password/factor minima and localized errors. Parent review added the disabled TanStack-query regression assertion, and the combined focused account/admin/content gate passes `61/61` with the staff-login validator at `12/12`, web typecheck and formatting. Live authenticated, provider, storage, browser-pixel and full-AT gates remain separate.
- Latest WEB-002/004/005 page-local parallel wave (2026-09-14): Anscombe kept cached catalog data visible while exposing a localized stale-refresh warning and retry action; Tesla clears route-derived checkout address/shipping/coupon state when an in-place route omits those values, while preserving safe idempotency-key retry behavior; and Goodall trims decoded public content slugs to match the API normalization contract. Parent review fixed the catalog test's type-only import; the focused catalog/checkout/content gate passes `24/24`, source/plain suites pass `435`/`693` with `1`/`2` opt-in MinIO skips, typecheck/lint, targeted formatting, full build, release verification, deterministic integration and live preflight pass. The elevated pinned-Chromium matrix was rerun after these fixes and passed `68/68` in `27` files. Content files retain pre-existing whole-file Prettier drift; authenticated/provider, formal browser-pixel and full-AT gates remain separate.
- Latest ADMIN-002 page-local follow-up (2026-09-14): Peirce added a dedicated Persian label map for every payment-attempt status rendered in order details, preserving an explicit fallback for unknown values. The focused admin order-page suite passes `7/7`; source/plain suites pass `437`/`695` with `1`/`2` opt-in MinIO skips, typecheck/lint, targeted formatting, production build, release verification, deterministic integration and live preflight pass, and the elevated pinned-Chromium matrix passes `68/68` in `27` files. The browser evidence remains fixture-backed; authenticated staff operations, formal mobile pixel comparison and full AT remain separate gates.
- Latest bounded local bug-fix wave (2026-09-14): Leibniz synchronizes the checkout new-address form with route changes so a removed `newAddress=1` marker cannot leave stale UI state; Laplace remounts inventory detail when the selected variant route identity changes; and Kuhn binds an existing refund idempotency key to its persisted order/provider/amount/payment-attempt/return-request identity before retrying a gateway call. Focused checkout tests pass `14/14`, the page-level inventory regression passes `10/10`, payment refund/service tests pass `9/9`, full admin tests pass `62/62`, source/plain suites pass `440`/`699` with `1`/`2` opt-in MinIO skips, and typecheck/lint, production build, local-release verification, deterministic integration, live preflight `12/12`, configured signal probe `5/5` and elevated fixture-backed Chromium `68/68` all pass. Payment/provider sandbox, authenticated staff, formal AT/pixel, restore/deploy, valid disposable concurrency and remote gates remain open; no credentials, provider call, commit, merge or push was used.
- Latest provider/auth/cart boundary wave (2026-09-14): Boole made unknown `NODE_ENV` values fail closed instead of selecting zero-cost local shipping; Noether redacts transport-thrown `ServiceUnavailableException` details at the SMS.ir OTP boundary; and Nietzsche prevents guest-cart controls from being disabled when guest merge is explicitly off. The focused provider/auth/cart gate passes `30/30`, source/plain suites pass `443`/`704` with `1`/`2` opt-in MinIO skips, targeted Prettier, typecheck/lint, production build, local-release verification and elevated fixture-backed Chromium `68/68` pass. The repository-wide format check remains red on `119` pre-existing files; authenticated/provider, restore/deploy, formal AT/pixel and remote gates remain open. No credentials, provider call, commit, merge or push was used.
- Historical live concurrency continuation (2026-09-14): before the safety follow-up, the documented PostgreSQL 16 reservation race temporarily mutated the persistent `nova-local`/`nova` inventory baseline, produced exactly one fulfilled and one insufficient-stock rejection, verified the persisted reservation and stock movement, restored the exact inventory baseline, and left zero synthetic rows. The harness now rejects that persistent target before any database client is constructed; future runs require the documented disposable database. No service restart or external provider call was used.
- Current live concurrency verification (2026-09-14): the guarded `test:concurrency` harness ran against a freshly created `nova_concurrency_validation_<run-id>` PostgreSQL database after all seven migrations and seed. It produced exactly `fulfilled=1` and `rejected=1`, persisted one active reservation and one reservation stock movement with `onHand=1/reserved=1`, deleted its synthetic rows, restored the exact disposable-row baseline, dropped the database, and a follow-up catalog query confirmed the target no longer exists. No shared `nova` data or external provider was used.
- Live notification continuation (2026-09-14): the new `bun run test:notification-live` harness was executed against a fresh loopback PostgreSQL database named under the required `nova_worker_validation_` prefix after all seven migrations were applied. One provider-unconfigured `processNotificationBatch` tick produced `claimed=1`, `retried=1`, `attempts=1`, `PENDING`, stable `notification-delivery-failed`, and `processedAt=null`; cleanup deleted one synthetic row and verified zero residue, then the disposable database was dropped. No provider call, long-lived worker, or shared `nova` outbox mutation was used.

Continuation fixes (2026-09-14): CSRF guard coverage is `6/6` for safe-method normalization, valid-cookie rotation idempotence, malformed-cookie tolerance and `SkipCsrf`; the date-sensitive delivered-return fixture now uses a relative timestamp and focused orders tests pass `5/5`; staff-auth and API-client contract suites pass `12/12` and `16/16`, with direct role-guard, successful staff-cookie/envelope, malformed/expired/revoked session and `CustomerUser`/`StaffUser` field-set parity coverage. The PostgreSQL backup verifier restores ambient `PG*` variables after setup failures, with parser/static checks passing. `docs/design-qa.md` now records the current `68/68` fixture-backed matrix, `12/12` local preflight and explicit real-auth/AT/pixel blockers. These focused checks did not mutate application data; the separate reservation check temporarily changed and restored the shared inventory baseline and left no residue, while the notification check used a separate disposable database. Docker/WSL-backed PostgreSQL, Redis, MinIO, API and storefront services are now healthy after recovery.

Latest AUTH guardrail follow-up (2026-09-13): deterministic staff-auth coverage now passes `9/9` for staff-cookie selection, invalid/missing session rejection, request attachment, idempotent logout, customer-cookie isolation, ADMIN revocation, cookie clearing and null envelopes. The API-client contract suite now passes `16/16` with an exact assertion that staff logout is CSRF-only and advertises no `staffSession`, `x-nova-auth` or `401`. No production behavior or OpenAPI artifact changed; real staff/MFA and external gates remain open.

Latest follow-up (2026-09-13): TEST-001 added a synthetic customer OTP-verification `401` recovery test that keeps `#auth/verify` stable, renders the localized error, re-enables retry, and leaves session storage/mutations safe; parent focused validation is `1/1`, discovery is `68` tests in `27` files, and the full pinned-Chromium matrix is `68/68`. OPS-002 also corrected the post-restore user-table count to exclude `pg_catalog`, `information_schema` and `pg_toast`, with parser/static/namespace/runbook assertions passing. No credentials, live provider/database/storage/restore operation, commit, merge or push was used; real auth/MFA, provider, authenticated media, full restore/deploy, AT/pixel and remote gates remain open.
...
Baseline: `master @ 3f447fbe6d76614382f3deca1721a6eaef04a485` (current verified integrated baseline; DB-001 and SEC-001 are included).

Historical remote sync update (2026-09-12; superseded by the 2026-09-14 recheck): read-only GitHub access was available. `origin/master` resolved to `f6b917f02275ef81a36f492d9ba914e48c913707`, matching local `master` at that snapshot; PR #2 and PR #4 were merged, while PR #5 was open and `CONFLICTING`. No remote write was performed because push/PR mutation requires explicit authorization.
...
- `ADMIN-001` — 🟡 page-local catalog/inventory slice is implemented at task commit `0daabff` and locally integrated into `codex/integration` via cherry-pick `a0146b7` plus parent route/fix commit `ba864c6`. The bounded direct product-by-ID follow-up is now implemented in API commit `79ba858` and web commit `4e29bc2`: staff-role-protected `GET /v1/admin/catalog/products/{productId}` and a per-product browser query replace the old list-only lookup. The inventory detail panel now preserves the selected-row loading skeleton and retryable initial error before data exists while keeping the disabled-query empty state, and the product editor remounts when its route identity changes so product-scoped child state cannot cross records; focused catalog/inventory tests pass `14/14`, package typechecks and integration checks pass. Live authenticated operations and mobile/pixel QA remain open.
- `ADMIN-002` — 🟡 page-local order-operations slice is implemented at task commit `e11e087` and locally route-integrated on `codex/integration` via `2f533f2`; the order-detail refund panel now localizes all six payment-attempt statuses with an unknown-status fallback, and the focused admin order-page suite passes `7/7`. Live authenticated operations and pixel-level mobile QA remain open.
- `ADMIN-003` — 🟡 page-local support/finance inspection slice is implemented at task commit `3b9f58d` and locally integrated via `89cf11b` plus the shared route wiring `2f533f2`; role matching now tolerates case variation from a current-session role payload while preserving unknown-role denial; the focused combined admin inspection run passes `24/24`, while live authenticated inspection and mobile/pixel QA remain open.
- `ADMIN-004` — 🟡 page-local content/SEO/redirect slice is implemented at task commit `e525ac8` and locally integrated into `codex/integration` via cherry-pick `ede0745` plus parent route/fix commit `ba864c6`. Publishing is disabled while the editor has unsaved draft changes, preventing stale server content from being published; focused admin/content tests pass `18/18`, web typecheck, targeted lint/format, diff-check and the client+SSR build pass. Live authenticated mutations and mobile/pixel QA remain open.
- `TEST-001` — ✅ deterministic follow-up commit `44dad93` passed both independent review axes and parent validation, then was merged locally in `f34cbb5`; the 2026-09-11 and 2026-09-12 isolated runtime/concurrency evidence remains historical. The local Playwright suite covers the existing public/unauthenticated journeys, exact-viewport staff-login semantic/keyboard journeys, staff invalid-email/loading/API-error/401/429 states, authenticated support-role and customer/session boundaries, dashboard summary/layout/error/accessibility/empty states, customer order/OTP recovery, admin catalog reads, checkout recovery, anonymous admin routes, content/SEO/discovery routes, read-only admin operations, six-viewport accessibility/reduced-motion smoke checks, and selector-specific quantitative checks. The latest elevated matrix remains `68/68` in `27` files; source-scoped `bun run test` passes `450` with one opt-in MinIO skip across `78` files, and current plain `bun test` passes `715` with two opt-in MinIO skips across `121` files. The deterministic integration matrix passes 8 suites / 153 underlying tests, and the current live `test:e2e` preflight passes `12/12` against healthy local PostgreSQL/Redis/MinIO/API/storefront services. Authenticated real-credential flows, storage upload, external provider delivery, formal visual comparison and full AT remain open.
...
- `WEB-002` — 🟡 discovery/cart plus SSR hydration slices are locally integrated through `36efd32` and `47cb203`, with parent route integration in `465ce07`; the bounded guest-cart conflict recovery follow-up is integrated in `b091396`, `4b0aa61` and `382fec4`; category SSR/hydration now uses the existing `limit=8` query key, and cached catalog results now expose a localized stale-refresh warning with retry after background failure; the focused SSR/main suite passes `31/31`, the catalog follow-up is covered in the current `24/24` catalog/checkout/content gate, and web typecheck/lint/build pass, while live/browser/mobile QA remains open.
- `WEB-003` — 🟡 account/orders/returns slice is locally integrated through `5df3fb8` and `465ce07`; eligible return-status routes now render an explicit not-requested state with a request-creation link instead of blank content, OTP verification isolates protected customer caches while preserving guest carts, and submitted return responses are scoped to the current route order number. Focused account/order tests pass `10/10`, the auth cache regression passes `4/4`, and web typecheck/format pass. Live/authenticated/mobile QA remains open.
- `WEB-004` — 🟡 checkout/payment-recovery plus idempotency-recovery slice is locally integrated through `b7e18b4` and `465ce07`; the cart-scoped idempotency follow-up is integrated in `075b7bd`; a confirmed `503 SERVICE_UNAVAILABLE` unconfigured-payment response now gets a fresh-key retry, while generic server/network/offline/unknown failures retain same-key behavior; route-derived address/shipping/coupon state and the `newAddress=1` form marker now synchronize on in-place route transitions; focused checkout tests pass `14/14`, web typecheck/lint/format and build pass, while provider/live-payment/browser QA remains open.
- `WEB-005` — 🟡 public content/system slice is locally integrated through `e4b756b` and `465ce07`; decoded public slugs now trim after URL decoding to match the API normalization contract, with the focused content regression covered in the current `24/24` catalog/checkout/content gate. The live built-SSR/API smoke on 2026-09-12 rechecked catalog-backed home/product/category rendering, robots and sitemap responses, plus private/unknown noindex behavior; published CMS content, authenticated admin mutation, mobile and pixel QA remain open.
- Current local integration — `master` and `origin/master` now resolve to `3f447fb` and contain the locally integrated storefront, admin, content, SEO, provider, QA and release-readiness slices; `codex/integration` remains at `f6b917f` (one commit behind). No remote `codex/integration` branch exists, and no push or PR mutation was performed.
...
- CI — 🟡 the local workflow fix `32d105a` now generates the Prisma client before `Typecheck`, and the workflow now runs the deterministic `bun run test:integration` matrix after the workspace test step. The current clean-checkout-equivalent sequence (`bun run db:generate` followed by `bun run typecheck`) and the integration matrix (`8` suites / `153` underlying tests) pass locally. The historical `master`/PR #2 failures remain tied to remote revisions and cannot be re-confirmed or synchronized while repository export/remote access is blocked.
...
- **Shipping:** Iran Post for the production adapter plus a deterministic no-network local sandbox; `STANDARD=پیشتاز`, `EXPRESS=اکسپرس` only where supported, prepaid/online only, and no COD in V1. The real quote remains authoritative.
...
- **Parallel provider lane:** run `PROVIDER-001`, `PROVIDER-002` and `PROVIDER-004` with their approved sandbox/local inputs. Keep `PROVIDER-003` blocked until the official Iran Post HTTP and parcel contract is verified; its deterministic local adapter remains available.
...
| PROVIDER-003 | API-001, Iran Post and approved shipping policy                   | no                           | `codex/provider-003-shipping`          |
...
**Status:** ✅ follow-up contract revision is complete at `1f0237a` and already contained in the current local/remote `master` tip `3f447fb`; contract `14/14`, package tests `38/38`, focused orders/payments tests `6/6`, typecheck and build pass. PR #2 is merged remotely at `17c0de0`; no further merge is needed, and remote CI/release evidence remains a separate gate.
...
**Current follow-up (2026-09-14):** Return-review and refund status chips map the established API vocabulary to semantic tones, and the order-detail refund panel maps all six payment-attempt statuses to Persian labels with an unknown-status fallback. Focused page tests pass `7/7`; live authenticated operations and pixel-level mobile QA remain open.
...
**Current follow-up (2026-09-14):** Successful customer OTP verification now clears the protected address and order list/detail query prefixes before publishing the new customer, removes only a cached customer-owned cart, and preserves a guest cart. The focused auth boundary passes `4/4`; live/authenticated and mobile QA remain open.
...
**Current follow-up (2026-09-14):** Retryable checkout submission failures now retry the submission operation and reuse the existing idempotency path, while quote failures continue to refetch the quote. Route transitions also synchronize the `newAddress=1` marker so a new-address form cannot remain open after leaving that route. The focused checkout regression passes `14/14`; provider/live-payment and browser QA remain open.
...
**Status (2026-09-14):** 🟡 local deterministic gate strengthened: explicit `Status=NOK` callbacks now fail closed without provider verification, malformed/coercible response codes are rejected as non-numeric-safe-integer values, refund retries are bound to the persisted operation identity before a gateway call, and regression coverage confirms that a successful provider response cannot upgrade a callback to `PAID` or reuse a refund key for a different operation. The focused ZarinPal gateway suite passes `15/15`, and the payment refund/service suite passes `9/9`; the full unit matrix, integration matrix, typechecks, lint, build and live runtime smoke pass. ZarinPal sandbox credentials/transaction remain required for the external gate.
...
**Status (2026-09-15):** 🟡 deterministic local/test selection remains no-network; the Iran Post adapter trims configuration and requires an HTTPS production base URL before transport invocation, with focused regression coverage for unsafe URLs, sanitized transport failures, invalid money conversion/overflow and response-method mismatch (`16/16` shipping/config tests pass). The official Iran Post quote/ETA endpoint and parcel request contract remain unverified, so no speculative endpoint or payload was added and the live transport remains fail-closed.

**Decision:** Iran Post production adapter boundary plus a deterministic no-network local shipping sandbox; the approved province/method/pricing and return-shipping policy are recorded above. Provider credentials remain outside Git.
...
**PR:** `feat(PROVIDER-003): add Iran Post shipping provider boundary`

---

## PROVIDER-004 — object storage/media

**Status (2026-09-14):** 🟡 local composed gate strengthened: S3 SigV4 canonical headers/query values use bytewise ordering, media object IDs reject trailing line terminators/non-string runtime values, role-specific `original.*`/`derivative.*` object names are enforced across completion, derivative reads and quarantine, and the new opt-in media harness passes the real local HTTP+Prisma+MinIO presign/upload/complete/delete lifecycle with persisted `READY` state on a unique disposable database/bucket. Focused catalog/media lifecycle tests pass `38/38`, the OpenAPI presign/complete operations remain mapped to their producer DTOs with `15/15` contract coverage, and the elevated package/API/web-SSR/worker build plus local-release artifact checks pass. The local Compose contract pins the verified MinIO digest; synthetic staff/TOTP and MinIO evidence does not replace real staff/MFA, external provider or production-storage sign-off.
...
**Latest verification (2026-09-15):** The real non-production PostgreSQL 16.15 archive/restore drill passed with the PostgreSQL 16.15 client binaries, read-only local `nova-local` source, and a disposable Docker `postgres:16-alpine` target created from `template0` on a separate cluster. The verifier produced a `134871`-byte archive with `342` entries, restored `54` tables including `_prisma_migrations`, reported distinct cluster identities and exited `0`; the disposable target was removed after verification. The verifier also now selects the first resolved application executable when multiple PostgreSQL majors are installed in `PATH`. Parser/static/version/namespace/runbook assertions and the local-release fresh-build/`-SkipBuild` checks remain passing; production backup/restore, deployment and rollback evidence remain open.

**Status:** ✅ the CI prerequisite and bounded PostgreSQL backup/restore verifier are accepted and merged locally into `codex/integration` as `c4f2aa6`. The verifier task head is `05b5c89`; two final independent review axes accepted its environment-only credential boundary, safe database selection, pinned endpoint/cluster identity, atomic archive publication, broad target sanity checks and explicit operator-maintenance gate. The current workflow also validates its own formatting and runs the deterministic `bun run test:integration` matrix after the workspace test; both pass locally. The repository-owned `infra/deploy/verify-local-release.ps1` now passes artifact-only and full-build modes, covering the package, API, web SSR and worker launch artifacts. The local Compose contract pins MinIO to the verified immutable digest documented above, and the current three-service runtime is healthy after the documented local start reconciled the S3 container. The 2026-09-12 decision register now fixes CI/CD promotion, host-managed secrets, production image digests, manual production approval and previous-digest rollback. No external object-storage backup is required for the current MinIO scope; Docker volume persistence is not represented as backup. No remote PR was created because repository export/push access is blocked. Live deployment/rollback evidence and the remaining monitoring/recovery gates remain open.
...
**Latest verification (2026-09-14):** The source-scoped root suite passes `450` with `1` opt-in MinIO skip and `0` failures across `78` files; the current plain `bun test` invocation passes `715` with `2` opt-in MinIO skips and `0` failures across `121` files. Workspace typecheck/lint, deterministic integration (8 suites / 153 underlying tests; 0 failures), and both local-release verifier modes pass; fresh mode includes Prisma generation and package/API/web-SSR/worker artifact plus reparse-point checks. The fixture-backed elevated pinned-Chromium matrix is `68/68` in `27` files; separately, the current live `test:e2e` preflight is `12/12` and the configured read-only API/Redis/MinIO/worker signal probe passes `5/5` against healthy local services. Protected real-auth, storage-upload and external-provider gates remain separate.

**Current parent revalidation after the second bounded auth/account wave (2026-09-14):** source/plain suites pass `450`/`715` with `1`/`2` opt-in MinIO skips across `78`/`121` files; the focused second-wave gate passes `40/40`; typecheck/lint, deterministic integration (8 suites / 153 underlying tests), elevated production build, local-release verification and fixture-backed pinned-Chromium `68/68` pass. The unprivileged build still hits the known Windows esbuild access boundary before source compilation, while live preflight `12/12` and the five-signal probe pass. No credentials, provider calls, shared-data mutation, commit, merge or push were used.

**Status:** ✅ follow-up commit `44dad93c54c3510241b1d7cba1db2348443520c7` passed both independent review axes and parent validation, then was merged locally in `f34cbb5`. Parent validation passed runner tests `3/3`, typecheck, lint, format and the full deterministic matrix (`8` suites, `131` tests) at that historical snapshot; the current integrated matrix is `8` suites and `153` underlying tests. The earlier 2026-09-11/12 isolated runtime evidence remains historical, while the current guarded concurrency run passed on a fresh disposable PostgreSQL database with exact cleanup and teardown. The latest elevated Playwright matrix remains `68/68` with one worker in `27` files. Source-scoped `bun run test` passes `450` with one opt-in MinIO skip across `78` files; current plain `bun test` passes `715` with two opt-in MinIO skips across `121` files. The current live `test:e2e` preflight passes `12/12` against healthy local PostgreSQL/Redis/MinIO/API/storefront services and the configured read-only API/Redis/MinIO/worker signal probe passes `5/5`. Authenticated flows and external provider delivery remain `BLOCKED` or `NOT RUN`.
...
**Accept:** expose `test:integration`/`test:e2e` only when real harness exists; critical races/duplicates executable; browser tests prove runtime wiring, not component existence. The separate `test:concurrency` command is a live, mutating disposable-PostgreSQL check and requires an explicit loopback `DATABASE_URL` whose database name matches `nova_concurrency_validation_<run-id>`; it does not claim provider or browser coverage.
...
**Latest verification (2026-09-14):** The browser coverage additions remain fixture-backed and read-only: customer empty-order and customer OTP `401` recovery checks pass, and the latest elevated pinned-Chromium matrix remains `68/68` across `27` files. The current live `test:e2e` preflight passes `12/12` against healthy local PostgreSQL/Redis/MinIO/API/storefront services after Docker/WSL recovery. Full screen-reader/AT, formal pixel comparison and real authenticated/provider state journeys remain `NOT RUN` or `BLOCKED`.
...
4. Run `PROVIDER-001`, `PROVIDER-002` and `PROVIDER-004` in parallel with the approved non-production inputs. Do not substitute an invented Iran Post contract for the `PROVIDER-003` blocker.
...
No blocking product/provider decision remains from the 2026-09-12 grill. The current local integration branch and checkout are `master` at `3f447fb`; `codex/integration` remains at `f6b917f` (one commit behind). The supplied Atelier references remain the design inputs where applicable.
