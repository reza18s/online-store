...
- Current local integration — `codex/integration` contains the locally integrated storefront, admin, content and SEO slices plus the ADMIN-001 direct-detail contract follow-up (`79ba858` + `4e29bc2`); the exact final HEAD/ahead count is recorded in the live status ledger after this documentation update. The parent branch has not been pushed because repository export/remote synchronization remains unavailable.
